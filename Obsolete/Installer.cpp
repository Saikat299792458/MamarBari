    ...
    switch (u)
    {
        case WM_CREATE:
        {
            CreateWindowEx(WS_EX_WINDOWEDGE, "Static",
                           "Select a Destination folder of installation:",
                           WS_CHILD | WS_VISIBLE, 10, 20, 500, 40, h,
                           (HMENU)ID_STATIC, NULL, NULL);
            HWND he = CreateWindowEx(
                          WS_EX_CLIENTEDGE, "edit", "Browse",
                          WS_CHILD | WS_VISIBLE | ES_NOHIDESEL | ES_AUTOHSCROLL, 75, 102,
                          250, 26, h, (HMENU)ID_EDITBOX, NULL, NULL);
            strcpy(path, defpath);
            SetWindowText(he, path);
            SendMessage(he, EM_SETSEL, 0, 100);
            CreateWindowEx(WS_EX_WINDOWEDGE, "Button", "Browse...",
                           WS_CHILD | WS_VISIBLE, 75 + 250, 100, 100, 30, h,
                           (HMENU)ID_BROWSE, NULL, NULL);
            CreateWindowEx(WS_EX_WINDOWEDGE, "Button", "Install",
                           WS_CHILD | WS_VISIBLE | BS_DEFPUSHBUTTON, 300, 200,
                           80, 25, h, (HMENU)ID_INSTALL, NULL, NULL);
            CreateWindowEx(WS_EX_WINDOWEDGE, "Button", "Cancel",
                           WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 380, 200, 80,
                           25, h, (HMENU)ID_CANCEL, NULL, NULL);
        }
        break;
            ...
            switch (LOWORD(w))
            {
                ...
                case ID_INSTALL:
                    ...
                    CreateWindowEx(
                        ...
                        "Installation in Progress:", WS_CHILD | WS_VISIBLE, 10,
                        20, 500, 20, h, (HMENU)ID_STATIC, NULL, NULL);
                    CreateWindowEx(WS_EX_WINDOWEDGE, "Button", "Finish",
                                   WS_CHILD | WS_VISIBLE | WS_DISABLED, 300,
                                   200, 80, 25, h, (HMENU)ID_FINISH, NULL,
                                   NULL);
                    CreateWindowEx(WS_EX_WINDOWEDGE, PROGRESS_CLASS, "",
                                   WS_CHILD | WS_VISIBLE, 10, 50, 470, 25, h,
                    ...
                    CreateWindowEx(WS_EX_WINDOWEDGE, "Button", "Run MamarBari",
                                   WS_CHILD | WS_VISIBLE | BS_CHECKBOX, 10, 90,
                                   250, 25, h, (HMENU)ID_RUN, NULL, NULL);
                    SetWindowText(GetDlgItem(h, ID_STATIC),
                                  "Installation Completed");
                    SendMessage(GetDlgItem(h, ID_RUN), BM_SETCHECK, BST_CHECKED,
                                0);
                    RECT r1;
...
APIENTRY WinMain(HINSTANCE a, HINSTANCE b, LPSTR c, int d)
{
    ...
    HWND h = CreateWindowEx(WS_EX_CLIENTEDGE, "INSTALLER", "MamarBari Setup",
                            WS_OVERLAPPEDWINDOW ^ WS_THICKFRAME, (x - 500) / 2,
                            (y - 300) / 2, 500, 300, NULL, NULL, a, NULL);
